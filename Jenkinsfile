...
node {
  ...
  try {
    ...
    // Prevent build running normally until service name is modified.
    // This will avoid conflicts between the scheme and plan services.
    if (serviceName != 'ffc-elm-scheme-service') {
      stage('Fail build') {
        sh "exit 1"
      }
    } else {
